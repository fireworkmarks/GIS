...
//滑轮放大缩小 背景图变换
    ...
    function windowHeight() {
	    ...
	    h=h+300;
	    var h1 = document.body.clientHeight; //获取当前窗口可视操作区域高度
...
//判断是否为空
	function checkForm()
	{
		...
		if (userPhone != null && userPhone != "") {
			if(isNaN(userPhone)){
				document.getElementById("error").innerHTML = "电话必须为数字！或者不填";
				return false;
			}
			if (userPhone.length!=11) {
				document.getElementById("error").innerHTML = "电话必须为11位！或者不填";
				return false;
			}
		}
		...
		if (password1.length<6) {
			document.getElementById("error").innerHTML = "密码不小于6位";
			return false;
		}
